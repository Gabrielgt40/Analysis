...
## Álcool
```{r}
dados2 <- read_excel("dados2.xlsx",2)

modelo7 <- aov (mort ~ trat, data = dados2)
anova(modelo7)

plot(simulateResiduals(modelo7))

means_modelo7 <- emmeans (modelo7, ~ trat, type="response")
means_modelo7

## Comparação dos tratamentos
emms <-  emmeans(means_modelo7, ~ trat, type = "response")
cld(emms, interaction = "pairwise", Letters = LETTERS)
```


```{r}
dados2 <- read_excel("dados2.xlsx",4) %>% 
  mutate(trat = as.factor(trat))

dados2 %>% 
ggplot(aes(trat, mort)) + 
  geom_point()+
  geom_smooth(method = "auto", formula = y ~ x, se = FALSE) +
  stat_poly_eq(formula = y ~ x, eq.with.lhs = "italic((y))~`=`~", aes(label = paste(..eq.label.., ..rr.label.., sep = "~")), parse = TRUE) +
  labs(y = "Mortalidade (%)",
       x = "Doses") +
  theme_classic() +
...
## Diâmetro
```{r}
dados3 <- read_excel("dados2.xlsx",3)

modelo7 <- aov (dam ~ trat, data = dados3)
anova(modelo7)
